...
struct SlugifyTests {
    ...
    func slugifyBlogTitles() {
        #expect(
            slugify("Comment j'ai installé Python sur Mac")
                == "comment-j-ai-installe-python-sur-mac")
        #expect(
            slugify("REST: Méthodes et status code HTTP") == "rest-methodes-et-status-code-http")
        #expect(
            slugify("Git - Comment je squash mes commits") == "git-comment-je-squash-mes-commits")
    }
