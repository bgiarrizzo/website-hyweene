...
public class BlogGenerator: Generator {
    ...
    private func loadPosts() throws {
        ...
        self.posts = loadedPosts.sorted {
            guard let date1 = $0.publishDate?.original,
                let date2 = $1.publishDate?.original
            else {
                return false
            }
            return date1 > date2
        ...
        let yearGroups = postsByYear.keys.sorted(by: >).map { year in
            [
                ...
                "posts": postsByYear[year]!
                    .sorted {
                        ($0.publishDate?.original ?? Date()) > ($1.publishDate?.original ?? Date())
                    }
                    .map { $0.toDictionary() },
            ]
        }
    ...
    private func writeCategoryPages() throws {
        let allCategories = categories.map { $0.toDictionary() }
        let operations = categories.map { category in
            {
                let filteredPosts = self.posts.filter { $0.category?.slug == category.slug }
                ...
                let yearGroups = postsByYear.keys.sorted(by: >).map { year in
                    [
                        ...
                        "posts": postsByYear[year]!
                            .sorted {
                                ($0.publishDate?.original ?? Date())
                                    > ($1.publishDate?.original ?? Date())
                            }
                            .map { $0.toDictionary() },
                    ]
                }
        ...
        try runConcurrently(operations: operations)
    }
